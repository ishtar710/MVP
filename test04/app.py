...
from typing import Dict as _D, Any as _A, List as _L

import gpv_parser
from app_common import build_plantuml, try_render_plantuml_png, DEFAULT_PARTICIPANTS  # type: ignore

st.set_page_config(page_title="Trace2UML Demo (No-dup RTT)", layout="wide")
st.title("Trace2UML Demo (No-dup RTT)")
...
def build_steps(events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for ev in events:
        et = ev.get("eventType")
        actor = (ev.get("actor") or "").upper()

        if et in ("HTTP_REQUEST_HEADER", "HTTP_REQUEST_BODY", "DECRYPTED_BODY"):
            continue

        if et == "EXT_HTTP_REQ" and actor:
            idx = len(steps)
            steps.append({"from": "GPV", "to": actor, "label": f"{actor} call", "note": None, "p95": None})
            last_out_idx[actor] = idx
            continue

        if et == "EXT_HTTP_RTT" and actor:
            if actor in last_out_idx:
                steps[last_out_idx[actor]]["p95"] = ev.get("durationMs")
            else:
                idx = len(steps)
                steps.append({"from": "GPV", "to": actor, "label": f"{actor} call", "note": None, "p95": ev.get("durationMs")})
                last_out_idx[actor] = idx
            continue
        ...
        if et == "EXT_HTTP_RESP" and actor:
            ...
            if actor in last_out_idx:
                del last_out_idx[actor]
            continue

        if et == "SYS_ERROR":
            steps.append({"from": "GPV", "to": "GPV", "label": "ERROR", "note": (ev.get("raw","")[:200] or None), "p95": None})
            continue
...
if "events" in st.session_state:
    ...
    st.download_button("코드 다운로드 (.puml)", data=puml.encode("utf-8"), file_name=f"{tx_choice}.puml")
